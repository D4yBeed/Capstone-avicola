...
export class HomePage implements OnInit {
  ...
  // 🔹 MÉTODO NUEVO: evita valores negativos en el input manual
  updateCount(key: EggKey, event: any) {
  if (this.form.disabled) return;

  // Valor original ingresado
  let raw = event.detail.value ?? '';

  // Convertir a string y limpiar cualquier cosa que NO sea dígito
  let cleaned = String(raw).replace(/[^0-9]/g, '');

  // Convertir a número
  let value = Number(cleaned);

  // Si está vacío o es NaN, dejar en 0
  if (!cleaned || isNaN(value)) {
    value = 0;
  }

  // Guardar el valor limpio en counts
  this.counts[key] = value;

  // 🔥 IMPORTANTE → Actualizar lo que aparece en el input forzando valor limpio
  ...
  async dec(key: EggKey) {
    try {
      if (this.form.disabled) return;
      if ((this.counts[key] || 0) === 0) return; // ⛔ nunca bajar de cero
